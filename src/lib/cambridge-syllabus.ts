 ...
 */
export function getCambridgePrompt(subject: string, grade: string, topic: string, numQuestions: number, topicObjectives: string[] = [], topicNumber = '', unitTitle = ''): string {
  const normalizedSubject = subject.trim();
  const normalizedGrade = grade.trim();
  const normalizedTopic = topic.trim();
  const normalizedTopicNumber = topicNumber ? topicNumber.trim() : '';
  const normalizedUnitTitle = unitTitle ? unitTitle.trim() : '';
...
${normalizedTopicNumber ? `The topic number is ${normalizedTopicNumber}.` : ''}
${normalizedUnitTitle ? `This topic belongs to the unit titled: "${normalizedUnitTitle}".` : ''}

Guidelines:
1. Target the ${normalizedGrade} cognitive level. Keep language simple, clear, and direct.
2. Align questions with Cambridge Primary learning objectives. ${topicObjectivesText}
3. ${subjectGuidelines}
