...
import ResultsClient, { MarkRecord } from '../../student/results/ResultsClient'
import Link from 'next/link'

export default async function ParentAcademicReportsPage({
  searchParams
}: {
  ...
  // Fetch released marks for this child
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }} className="no-print">
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
            View official academic results and report cards.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0 }}>
          Report Card for: <span style={{ color: 'var(--color-secondary)' }}>{selectedChild?.profiles.first_name} {selectedChild?.profiles.last_name}</span>
        </h3>
      </div>
