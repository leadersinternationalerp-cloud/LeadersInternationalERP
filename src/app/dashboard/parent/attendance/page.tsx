...
import AttendanceClient from '../../student/attendance/AttendanceClient'
...
export default async function ParentAttendancePage({
  ...
  const { data: relations } = await supabase
    ...
    .select('student_id, students:student_id (id, student_id, grade_level, profiles:id (first_name, last_name))')
  ...
  // Fetch all attendance for this selected child
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
            Monitor daily attendance for your children.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0 }}>
          Viewing records for: <span style={{ color: 'var(--color-secondary)' }}>{selectedChild?.profiles.first_name} {selectedChild?.profiles.last_name}</span>
        </h3>
        {/* We rely on browser print for now, a proper client component button would do window.print() */}
      </div>
