...
export interface HOSMarksRecord {
  id: string
  score: number | null
  term: string
...
export default function HOSMarksClient({ marks }: { marks: HOSMarksRecord[] }) {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {filtered.map(m => {
              ...
              const isMissing = m.score === null || m.score === undefined

              return (
                <tr key={m.id} style={{ borderBottom: '1px solid var(--color-border)', backgroundColor: isMissing ? 'rgba(239, 68, 68, 0.05)' : 'transparent' }}>
                  ...
                  <td style={{ padding: '1rem', fontWeight: 600, color: isMissing ? 'var(--color-error)' : 'var(--color-text)' }}>
                    {isMissing ? 'Missing' : `${m.score}%`}
                  </td>
                  <td style={{ padding: '1rem' }}>
                    <span style={{
                      padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 600,
                      backgroundColor: grade === 'A' ? 'rgba(16, 185, 129, 0.1)' : grade === 'F' || grade === 'N/A' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(0,0,0,0.05)',
                      color: grade === 'A' ? 'var(--color-success)' : grade === 'F' || grade === 'N/A' ? 'var(--color-error)' : 'var(--color-text)'
                    }}>
                      {grade}
                    </span>
                  </td>
