...
export default async function PrincipalAdvanceRequestsPage() {
  ...
  if (profile?.role !== 'Principal' && profile?.role !== 'System Admin') {
    return (
      <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <p>You do not have permissions to review staff advances.</p>
      </div>
    )
  }
  ...
  const { data: reviewedAdvances } = await supabase
    .from('salary_advances')
    .select(`
      *,
      employee:employee_id (first_name, last_name, role)
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '2rem', alignItems: 'start' }}>
        ...
        <div className="glass-panel" style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          <div style={{ padding: '1rem', borderBottom: '1px solid var(--color-border)', backgroundColor: 'rgba(0,0,0,0.02)', fontWeight: 600 }}>
            Pending Salary Advances ({pendingAdvances?.length || 0})
          </div>
              ...
              <div key={adv.id} style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                  <div>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>Role: {adv.employee?.role}</span>
                  </div>
        ...
        <div className="glass-panel" style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '1rem' }}>
            ...
            {(!reviewedAdvances || reviewedAdvances.length === 0) && (
              <p style={{ color: 'var(--color-text-muted)', textAlign: 'center', padding: '1rem' }}>
